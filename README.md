...
[![macOS native](https://img.shields.io/badge/macos%20native-2.1.113--2.1.207-green)](./docs/support-matrix.md)
...
| macOS · native binary（arm64） | `2.1.113 - 2.1.207` 内的已验证版本 | 需要 `node-lief`；个别版本未收录，见支持矩阵 |
...
> **native binary 说明**：官方安装器和新版 npm 包装到的是 native 二进制。插件会提取其中的 JS → 翻译 → 写回，并做启动自检；补丁、重打包或自检失败会恢复原文件。macOS arm64 已验证 `2.1.113 - 2.1.207` 内的版本（完整清单见[支持矩阵](./docs/support-matrix.md)）；更高版本也会本机自检，需要 `node-lief`。macOS 可在新会话安全补丁；Windows 不热改运行中的 exe，更新后需关闭窗口并重跑 `install.ps1`。
