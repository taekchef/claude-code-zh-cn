...
| Linux / 官方安装器 / native binary | `experimental` | `2.1.112`、`2.1.170` | 当前 Linux x86-64 ELF native 已验证 extract / patch / repack / `--version`；需要 `node-lief`；未验证新版本会安全跳过 CLI Patch |
...
>
> **Linux native binary experimental**：Linux x86-64 ELF native binary experimental；需要 node-lief；仅代表列出的已验证版本 `2.1.112`、`2.1.170`，已通过 extract / patch / repack / `--version` 端到端验证，不代表 future latest 自动稳定。未验证的 latest 会跳过 CLI Patch。
>
...
| Linux native binary `2.1.112`、`2.1.170` | 当前 Linux x86-64 ELF 已验证 extract / patch / repack / `--version` | `experimental`（需要 `node-lief`） |
