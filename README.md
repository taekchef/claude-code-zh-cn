...
> **native binary 说明**：官方安装器和新版 npm 包装到的是 native 二进制。插件会提取其中的 JS → 翻译 → 写回，并做启动自检，失败自动回滚。macOS arm64 已验证 `2.1.113 - 2.1.205` 内的版本（完整清单见[支持矩阵](./docs/support-matrix.md)），需要 `node-lief`。
