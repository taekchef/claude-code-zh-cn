...
- ✅ Patch cli.js 硬编码文字（1509 条翻译，1485 处有效 patch）
...
| UI 文字全量中文化 | 1509 条翻译，1485 处有效 patch | CLI Patch（逐条正则匹配双引号字符串） |
...
├── cli-translations.json    ← 1509 条 UI 翻译对照表
...
It translates 187 spinner verbs, 41 spinner tips, 1509 UI translations (1485 effective patches on Claude Code 2.1.96), notification messages, and more. The patch uses per-entry regex matching on double-quoted string boundaries, with regex literal exclusion via offset checks. After Claude Code updates, the plugin automatically re-patches on next session start. Supports macOS, Linux, and Windows (via WSL).
