...
- ✅ 已验证版本直接使用公开证据；同一 `major.minor` 新版先本机自检。可 patch 硬编码文字（1895 条翻译；代表版本 `2.1.112` 实测 1595 处有效 patch）
- ✅ 缺少 `node-lief`、跨版本线或自检失败时，只跳过 Layer 4；Layer 1~3 和 Claude Code 本体继续可用
...
| `powershell -File install.ps1` | Windows：旧 npm cli.js 最完整；native .exe `2.1.113 - 2.1.205` 内已验证版本需 `node-lief` |

> **native binary 说明**：官方安装器和新版 npm 包装到的是 native 二进制。插件会提取其中的 JS → 翻译 → 写回，并做启动自检；补丁、重打包或自检失败会恢复原文件。macOS arm64 已验证 `2.1.113 - 2.1.205` 内的版本（完整清单见[支持矩阵](./docs/support-matrix.md)），同一 `major.minor` 新版可本机自检，需要 `node-lief`。
...
Claude Code 更新后，插件会在首次会话启动时**自动检测版本变更并重新 patch**。同一 `major.minor` 新版先本机自检；跨版本线只跳过原生 Layer 4，不会让 CLI 失效。

插件本体发布新 Release 后，正式安装态由 Claude Code 插件管理器更新；独立兜底安装继续只跟随已发布 Release。本地源码安装用户也可手动运行：`git pull && ./install.sh`（Windows：`git pull` 后重跑 `install.ps1`）。
...
Layer 1~3（设置、Hook、插件）完全不受影响。Layer 4 会自动检测版本变更：同一 `major.minor` 新版在本机自检通过后，只翻译仍能精确匹配的文案；新文案保留英文。跨版本线不改 native 二进制。补丁、重打包或启动自检失败会恢复原文件——**CLI 始终可用**。已验证证据见 [docs/support-matrix.md](./docs/support-matrix.md)。
