...
function cliLooksPatched(cliFile, sentinelPattern) {
  try {
    const sample = fs.readFileSync(cliFile, "utf8").slice(0, 500_000);
    if (!sample.includes(sentinelPattern)) {
      return true;
    }
    return false;
  } catch {
...
function runDoctor(options = {}) {
  ...
  const sentinelPattern = "Quick safety check";
  ...
  if (kind === "npm" && target) {
    ...
    const patched = cliLooksPatched(target, sentinelPattern);
    if (!stable) {
      ...
      layer4Status = "needed";
      layer4Detail = "检测到未翻译的 UI  sentinel，CLI 可能仍是英文";
      ...
      layer4Status = "ok";
      layer4Detail = "cli.js 抽样未发现英文 sentinel";
...
module.exports = { runDoctor, STABLE_INSTALL_CMD, STABLE_PINNED_VERSION };
