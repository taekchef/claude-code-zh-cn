...
function usage() {
  return [
    "Usage: node scripts/prepare-native-failure-handoff.js --candidate candidate.json --output report.md [--text-report report.md] [--run-url URL] [--head-sha SHA]",
    "",
    "Writes a maintainer handoff report for a failed macOS native latest candidate.",
    "",
  ].join("\n");
}
...
function renderReport({ candidate, result, textReport, runUrl, headSha }) {
  ...
  return [
    `# Native latest candidate failure: ${version}`,
    "",
    `- Run: ${runUrl || "unknown"}`,
    ...
    "",
    "```bash",
    `node scripts/verify-upstream-compat.js --baseline ${version} --skip-latest --native-macos-arm64 --json`,
    `node scripts/generate-upstream-text-diff.js --to ${version} --native-macos-arm64`,
...
function main() {
  ...
  process.stdout.write(`prepared native failure handoff for ${version}\n`);
}
