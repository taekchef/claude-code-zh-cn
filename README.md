...
[![macOS native](https://img.shields.io/badge/macos%20native-2.1.113--2.1.170%20experimental-yellow)](./docs/support-matrix.md)
[![Linux native](https://img.shields.io/badge/linux%20native-2.1.172%20experimental-yellow)](./docs/support-matrix.md)
...
| macOS / native binary | `experimental` | `2.1.113 - 2.1.170`（不含未纳入本轮支持的 `2.1.115`、`2.1.125`、`2.1.127`、`2.1.130`、`2.1.134`、`2.1.135`、`2.1.147`、`2.1.149`、`2.1.151`、`2.1.154`、`2.1.155`、`2.1.157`、`2.1.163`、`2.1.164`、`2.1.166`） | 当前 macOS arm64 native 已验证 extract / patch / repack / `--version` + 11 个稳定显示面审计；需要 `node-lief`；未验证新版本会安全跳过 CLI Patch |
| Linux / npm 全局安装 | `stable` | `2.1.92 - 2.1.112` | 与 npm stable 同口径 |
| Linux / native binary | `experimental` | `2.1.172`（已验证）| 仅 2.1.172 在 linux-x64 实测通过；同 minor 线更高版本安装时会本机自验证，通过才启用 CLI Patch |
...
| Windows / native .exe | `experimental` | `2.1.113 - 2.1.162`（不含未纳入本轮支持的 `2.1.115`、`2.1.125`、`2.1.127`、`2.1.130`、`2.1.134`、`2.1.135`、`2.1.147`、`2.1.149`、`2.1.151`、`2.1.154`、`2.1.155`、`2.1.157`） | 当前 Windows x64 native 已验证 extract / patch / repack / `--version` + 11 个稳定显示面审计；需要 `node-lief`；未验证新版本会安全跳过 CLI Patch |
...
>
> **Windows native .exe experimental**：Windows x64 native binary experimental；需要 node-lief；仅代表列出的已验证版本 `2.1.113 - 2.1.162`（不含未纳入本轮支持的 `2.1.115`、`2.1.125`、`2.1.127`、`2.1.130`、`2.1.134`、`2.1.135`、`2.1.147`、`2.1.149`、`2.1.151`、`2.1.154`、`2.1.155`、`2.1.157`），已通过 extract / patch / repack / `--version` + 11 个稳定显示面审计，不代表 future latest 自动稳定。未验证的 latest 会跳过 CLI Patch；如需最稳，请使用 `npm install -g @anthropic-ai/claude-code@2.1.112`。
>
...
>
> **最新版说明**：Claude Code 从 `2.1.113` 开始，npm 主包切换为 native binary wrapper，不再包含旧的 `cli.js`。本插件当前 stable CLI Patch 支持到 `2.1.112`；macOS arm64 native binary 已验证 `2.1.113 - 2.1.170`（不含跳过版本）；Linux x86_64 native binary 新增 experimental 通道，仅 2.1.172 已实测；Windows x64 native 已验证 `2.1.113 - 2.1.162`（不含跳过版本）。未验证的新版本在安装时会本机自验证，通过才启用 CLI Patch。
...
| macOS native binary `2.1.113 - 2.1.170`（不含跳过版本） | 已验证的 native binary 版本，显示审计 11/11 PASS | `experimental`（需要 `node-lief`） |
| Linux native binary `2.1.172` | 仅 2.1.172 已实测 extract / patch / repack / `--version` | `experimental`（需要 `node-lief`） |
| `curl -fsSL https://claude.ai/install.sh \| sh` | 官方安装器 latest | `experimental / skipped`（只有明确验证版本会启用 CLI Patch） |
| `powershell -File install.ps1` | Windows PowerShell 安装（旧 npm cli.js 为 stable；Windows x64 native `2.1.113 - 2.1.162` 为 experimental，需要 `node-lief`） | `stable / experimental`（需 PowerShell 5.1+） |

安装脚本会自动检测安装方式，无需手动选择。

> **native binary 说明**：官方安装器和新版 npm 包都可能装到 native 二进制，不是旧 npm `cli.js`。本插件的处理方法是：用 `bun-binary-io.js` 提取二进制里的 JS → 复用 `patch-cli.sh` 翻译 → 再写回二进制。macOS arm64 已验证 `2.1.113 - 2.1.170`（不含跳过版本）；Linux x86_64 仅 2.1.172 实测通过，同 minor 线更高版本安装时会本机自验证。运行时需要 `node-lief`。要最稳，请优先使用 npm pinned 安装方式。
>
...
- ✅ 在 stable 安装方式上 patch 硬编码文字（1838 条翻译；当前 stable 代表版本 `2.1.112` 实测 1555 处有效 patch，显示审计 11/11 PASS）
- ✅ 在 macOS native experimental 已验证版本上 patch 硬编码文字（`2.1.113 - 2.1.170`，实测 1320-1435 处，显示审计 11/11 PASS）
- ✅ 在 Linux native experimental 已验证版本上 patch 硬编码文字（`2.1.110 - 2.1.172`，2.1.172 实测 1428 处 patch）
...
> **CLI Patch 支持范围**：install.ps1 可 patch 旧 npm cli.js 形态（`2.1.92 - 2.1.112`），也可在安装了 `node-lief` 时 experimental patch Windows x64 native `2.1.113 - 2.1.162`（不含 `2.1.115`、`2.1.125`、`2.1.127`、`2.1.130`、`2.1.134`、`2.1.135`、`2.1.147`、`2.1.149`、`2.1.151`、`2.1.154`、`2.1.155`、`2.1.157`）。检测到未验证 Windows native .exe 或缺少 `node-lief` 时，会明确跳过 CLI Patch，只启用 Layer 1~3（设置 + Hook + 插件）。如需最稳，请使用 `npm install -g @anthropic-ai/claude-code@2.1.112` 安装旧 npm 版本。
>
...
> 完整 187 个翻译见 [locales/zh-CN/verbs.json](./locales/zh-CN/verbs.json)
...
| UI 文字中文化 | 1838 条翻译，`2.1.112` 实测 1555 处有效 patch；macOS native experimental `2.1.113 - 2.1.114`、`2.1.116 - 2.1.124`、`2.1.126`、`2.1.128 - 2.1.129`、`2.1.131 - 2.1.133`、`2.1.136 - 2.1.146`、`2.1.148`、`2.1.150`、`2.1.152 - 2.1.153`、`2.1.156`、`2.1.158 - 2.1.162`、`2.1.165`、`2.1.167 - 2.1.170` 实测 1320-1405 处；固定显示面审计均为 11/11 PASS | CLI Patch（扫描真实双引号字符串 token 后逐条替换）+ 显示面审计 |
...
├── cli-translations.json    ← 1838 条 UI 翻译对照表
...
│   ├── index.json              ← 语言索引
│   └── zh-CN/
│       ├── manifest.json       ← 语言包元数据
│       ├── translations.json   ← 1838 条 CLI 翻译对照表
│       ├── verbs.json          ← 187 个中文 spinner 动词
│       ├── tips.json           ← 41 条中文 spinner 提示
│       ├── session-context.json ← 中文上下文指令
│       └── notifications.json  ← 通知翻译
├── scripts/
│   ├── install-json-helper.js  ← 安装辅助工具
│   ├── clawgod-patch.sh        ← ClawGod 可选支持
│   ├── verify-settings-sources.js ← settings 数据源校验
│   └── sync-doc-derived-counts.js ← 文档统计数据同步
├── plugin/
│   ├── core/
│   │   ├── i18n.js             ← 多语言核心模块
│   │   └── binary-io.js        ← 原生二进制 I/O
```
...
```bash
# 编辑 spinner 提示
vim locales/zh-CN/tips.json

# 编辑 spinner 动词
vim locales/zh-CN/verbs.json
```
...
可以！编辑 `locales/zh-CN/tips.json` 和 `locales/zh-CN/verbs.json`，然后重新运行 `./install.sh` 即可。
</details>
...
- 翻译改进 → 编辑 `locales/zh-CN/tips.json` 或 `locales/zh-CN/verbs.json`
...
It translates 187 spinner verbs, 41 spinner tips, 1838 UI translations, notification messages, and more. The patch combines safe string scanning for legacy `cli.js` plus an experimental native-binary path for verified versions: macOS `2.1.113`–`2.1.170`, Linux `2.1.110`–`2.1.172` (), and Windows `2.1.113`–`2.1.162` (with some excluded versions), now guarded by stable display-surface auditing. On Windows, a PowerShell install script (`install.ps1`) is also available for the old npm `cli.js` form (2.1.92–2.1.112). Current support windows are documented in [docs/support-matrix.md](./docs/support-matrix.md).
