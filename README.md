...
[![macOS native](https://img.shields.io/badge/macos%20native-2.1.123%20experimental-yellow)](./docs/support-matrix.md)
...
| macOS / native binary | `experimental` | `2.1.123` | 当前 macOS arm64 native 已验证 extract / patch / repack / `--version`；需要 `node-lief`；未验证新版本会安全跳过 CLI Patch |
...
>
> **最新版说明**：Claude Code 从 `2.1.113` 开始，npm 主包切换为 native binary wrapper，不再包含旧的 `cli.js`。本插件当前 stable CLI Patch 支持到 `2.1.112`；macOS arm64 native binary 现在有独立 experimental 通道，已验证 `2.1.123`。`latest` 不是 stable 承诺，未验证的新版本会跳过 CLI Patch。
...
| Claude Code native binary `2.1.123`（macOS arm64） | 当前已验证的 native binary 版本 | `experimental`（需要 `node-lief`） |
...
> **native binary 说明**：官方安装器和新版 npm 包都可能装到 native 二进制，不是旧 npm `cli.js`。本插件的处理方法是：用 `bun-binary-io.js` 提取二进制里的 JS → 复用 `patch-cli.sh` 翻译 → 再写回二进制。macOS arm64 `2.1.110`、`2.1.111`、`2.1.112`、`2.1.123` 已在临时目录验证通过；运行时需要 `node-lief`。要最稳，请优先使用 npm pinned 安装方式。
>
...
- ✅ 在 stable 安装方式上 patch 硬编码文字（1573 条翻译；当前 stable 代表版本 `2.1.112` 实测 1450 处有效 patch）
- ✅ 在 macOS native experimental 已验证版本上 patch 硬编码文字（`2.1.123` 实测 1262 处）
...
| UI 文字中文化 | 1573 条翻译，`2.1.112` 实测 1450 处有效 patch；macOS native `2.1.123` 实测 1262 处 | CLI Patch（扫描真实双引号字符串 token 后逐条替换） |
...
├── cli-translations.json    ← 1573 条 UI 翻译对照表
...
注意：Claude Code 从 `2.1.113` 开始，npm latest 切换为 native binary wrapper，不再包含旧的 `cli.js`。当前 stable CLI Patch 支持到 `2.1.112`。macOS arm64 native binary 走 experimental 通道，已验证 `2.1.123`；未验证 latest 不代表 stable 支持。
</details>
...
It translates 187 spinner verbs, 41 spinner tips, 1573 UI translations, notification messages, and more. The patch combines safe string scanning for legacy `cli.js` plus an experimental macOS arm64 native-binary path for explicitly verified versions such as `2.1.123`. On Windows, a PowerShell install script (`install.ps1`) is available for the old npm `cli.js` form (2.1.92–2.1.112); Windows native `.exe` / unverified latest builds are detected and skipped for CLI Patch (Layers 1–3 still active). Current support windows are documented in [docs/support-matrix.md](./docs/support-matrix.md).
