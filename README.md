...
[![npm stable](https://img.shields.io/badge/npm%20stable-2.1.92--2.1.104-green)](./docs/support-matrix.md)
...
| 系统 / 通道 | 当前口径 | 已验证窗口 | 说明 |
|------|---------|-----------|------|
| macOS / npm 全局安装 | `stable` | `2.1.92 - 2.1.104` | 启动前 launcher 自修复 + `session-start` 二层兜底 |
| macOS / 官方安装器 | `experimental` | `2.1.104` | 依赖 native repack；不走 npm launcher |
| Linux / npm 全局安装 | `stable` | `2.1.92 - 2.1.104` | 与 npm stable 同口径 |
| Linux / 官方安装器 | `unsupported` | - | 当前不承诺支持 |
| Windows / WSL + npm 全局安装 | 跟随 npm `stable` | `2.1.92 - 2.1.104` | **必须在 WSL 终端内运行** |
...
- ✅ 在支持的安装方式上 patch 硬编码文字（1462 条翻译；当前 stable 代表版本 `2.1.104` 实测 1368 处有效 patch）
