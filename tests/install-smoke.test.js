...
test("install smoke skips unverified native binaries instead of pretending CLI Patch succeeded", { skip: unixShellRequired }, () => {
  ...
  assert.match(output, /2\.1\.140/, "the user-facing message should include the unsupported version");
  ...
  assert.match(output, /2\.1\.113 - 2\.1\.139/, "the message should show the verified native window");
