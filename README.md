...
| Windows / npm 全局安装 (PowerShell) | `stable` | `2.1.92 - 2.1.112` | 使用 install.ps1，需 PowerShell 5.1+ |
| Windows / WSL + npm 全局安装 | 跟随 npm `stable` | `2.1.92 - 2.1.112` | **必须在 WSL 终端内运行** |

> **Windows 用户（原生）**：现已支持通过 PowerShell 脚本在 Windows 10/11 上原生安装，无需 WSL。见下方「Windows 原生安装」章节。
> 
...
| `powershell -File install.ps1` | Windows 原生 PowerShell 安装 | `stable`（需 PowerShell 5.1+） |
...
install.ps1 会自动完成与 install.sh 相同的7步操作：依赖检查、插件同步、Launcher 安装、Settings 合并、CLI Patch（仅 npm 安装）、元数据写入。
